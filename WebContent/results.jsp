			...
			<div class="col-md-12" id="map">
				<script type="text/javascript">
					...
					var map = L.map('map').setView(
							[ <c:out value="${leafletStationCoords}" /> ], 16);
					L
							.tileLayer(
					...
					var marker = L.marker(
							[ <c:out value="${leafletStationCoords}" /> ], {
								icon : railIcon
							}).addTo(map);
					marker
							.bindPopup(
									"<b>This is <c:out value="${leafletStationName}" /> Station.</b>")
							.openPopup();
					...
					marker
							.bindPopup("<b><c:out value="${location.name}"/></b><br><c:out value="${location.address}"/> ");
					</c:forEach>
				</script>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-md-5" id="searchcol">
				<form action="FavoritesServlet" method="get">
					...
					<button type="submit" class="btn btn-info btn-med btn-block">View
						Favorites</button>
				</form>
				<br>
